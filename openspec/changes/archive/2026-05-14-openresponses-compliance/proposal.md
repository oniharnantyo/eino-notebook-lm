...
### Known Issues (Post-Compliance Review)
- `response.in_progress` event missing required fields (object, model, truncation, parallel_tool_calls, text)
- Thinking tag parser doesn't handle HTML-encoded tags (`&lt;think&gt;`), causing tag leakage into message text

These issues are documented in section 6 of tasks.md with planned fixes.
